...
void* eventHandler(void* c); // replaces the worker_thread
...
int main(int argc, char * argv[])
{
    ...
    // pthread_t workerThreads[wThreads]; // worker threads not used in this MP
    ...
    /* Is replaced by the call to the event handler
    ...
    printf("Main Control Channel Closed\nAbout to print Histograms\n");
...
/**
 * @brief Will take the requests out of the request buffer and then send the name within
 *      that request to the server. Once the response from the server comes back, the
 *      thread will then put that reply into the appropriate statistics buffer
 * @param c
 * @return
 */
void* eventHandler(void* c)
{
    RequestChannel* channels[wThreads];
    int person_id[wThreads];
    ...
    for(int i = 0; i < wThreads; i++)
    {
        r = requestBuffer->RemoveFromBuffer();
        wCount++;
        channels[i]->cwrite(r.requestData);
        person_id[i] = r.requestID;
    }
    ...
    while(!finished)
    {
        ...
        if(select_result)
        {
            for(int i = 0; i < wThreads; i++)
            {
                ...
                {
                    printf("Reading\n");
                    string serverResponse = channels[i]->cread();
                    rCount++;
                    switch(person_id[i])
                    {
                        case 0:
                            joeBuff->AddToBuffer(Response(serverResponse, 0, NULL));
                            break;
                        case 1:
                            janeBuff->AddToBuffer(Response(serverResponse, 1, NULL));
                            break;
                        case 2:
                            johnBuff->AddToBuffer(Response(serverResponse, 2, NULL));
                            break;
                    }
    ...
    for(int i = 0; i < wThreads; i++)
    {
        printf("Destroying channel %d\n", i);
        channels[i]->send_request("quit");
        printf("Channel %d Destroyed\n\n", i);
    }
    printf("Sending quit request to main control channel within EVENT HANDLER\n");
    chan.send_request("quit");

    /* send kill signal to the statistics threads */
    joeBuff->AddToBuffer(*killSignal);
    janeBuff->AddToBuffer(*killSignal);
    johnBuff->AddToBuffer(*killSignal);
    printf("Returning from Event Handler\n");
    return (char*)"Event handler has finished";
}

/* TODO: find new way to kill the statistics thread */
/**
...
/**
 * @brief (THIS CODE IS NOW REPLACED BY AN EVENT HANDLER!!!) Will take the requests
 *      out of the request buffer and then send the name within that request to the
 *      server. Once the response from the server comes back, the thread will then
 *      put that reply into the appropriate statistics buffer
